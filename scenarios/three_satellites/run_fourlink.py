...
if __name__ == "__main__":
    ...
    if int(sys.argv[1]) == 0:
        ...
        length_list = np.linspace(0, 8800e3, num=96)
        length_cutoffs = [3800e3, 8800e3, 8800e3]
        custom_length_lists = [length_list[length_list <= len_cutoff] for len_cutoff in length_cutoffs]
        max_iter = 1e3
        cutoff_multiplier = 0.1
        num_processes = 32
        ...
        with Pool(num_processes) as pool:
            for t_dp, lens in zip(dephasing_times, custom_length_lists):
                ...
                t_params["T_DP"] = t_dp
                num_calls = len(lens)
                aux_list = zip(lens, [max_iter] * num_calls, [t_params] * num_calls, [cutoff_multiplier * t_params["T_DP"]] * num_calls, [num_memories] * num_calls, [first_satellite_multiplier] * num_calls)
