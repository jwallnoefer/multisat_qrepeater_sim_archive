...
for i, num_memories in memories.items():
    ...
    plt.xlabel("ground distance [km]")
    plt.ylabel("key per time [Hz]")
    plt.title(f"{scenario_str}: theta=2µrad, first_sat_multiplier=0.2, {num_memories=}")
...
plt.xlabel("ground distance [km]")
plt.ylabel("key per time [Hz]")
plt.title(f"{scenario_str}: theta=2µrad, first_sat_multiplier=0.2, num_memories=1000")
