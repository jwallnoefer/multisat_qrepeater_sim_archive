...
def run(length, max_iter, params, cutoff_time=None, mode="sim"):
    ...
    def state_generation(source):
        ...
        for idx, station in enumerate(source.target_stations):
            if station.memory_noise is not None:  # only central station has noisy storage
                #all sending related channels only apply to the bit flying from the endstation to the central station
    ...
    class TwoLinkProtocol(Protocol):
        ...
        def to_pandas(self, params, length, path='results'):
            data_frame = pd.DataFrame({'params':json.dump(params), 'length': length,  
                'time': self.time_list, 'fidelity_list': self.fidelity_list, 'cor_z': self.correlations_z_list, 
                'cor_x': self.correlations_x_list, 'resource_max': self.resource_cost_max_list})
            return data_frame
    ...
    source_A = SchedulingSource(world, position=0, target_stations=[station_A, station_central], time_distribution=time_distribution, state_generation=state_generation)
