...
x_base = np.arange(1000, 501000, 1000) / 1000
L_ATT = 22 * 10**3 / 1000  # attenuation length in km
eta = np.exp(-x_base / L_ATT)
y_repeaterless = 10 * np.log10(-np.log2(1 - eta))
y_optimal = 10 * np.log10(np.sqrt(eta))
...
y = [luet.lower_bound(l / 2) for l in x_base]
plt.plot(x_base / 1000, 10 * np.log10(np.array(y) / 2),
...
plt.minorticks_on()
plt.grid(which='both')
plt.savefig(os.path.join(result_path, "epp_multi_compare_1_2.png"))
plt.show()
