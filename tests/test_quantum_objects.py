...
test_four_qubit_channel = NoiseChannel(n_qubits=4, channel_function=lambda rho: test_noise_channel.apply_to(test_noise_channel.apply_to(rho=rho, qubit_indices=[1]), qubit_indices=[2]))
test_two_qubit_channel = NoiseChannel(n_qubits=2, channel_function=lambda rho: test_noise_channel.apply_to(test_noise_channel.apply_to(rho=rho, qubit_indices=[0]), qubit_indices=[1]))
...
class TestQuantumObjects(unittest.TestCase):
    ...
    def test_BSM_station_option(self):
        ...
        four_qubit_noise_model = NoiseModel(channel_before=test_four_qubit_channel)
        station_a = Station(world=self.world, position=0, BSM_noise_model=four_qubit_noise_model)
        pair_1a = Pair(world=self.world, qubits=[left_station.create_qubit(), station_a.create_qubit()], initial_state=test_state)
        pair_2a = Pair(world=self.world, qubits=[station_a.create_qubit(), right_station.create_qubit()], initial_state=test_state)
        ...
        two_qubit_noise_model = NoiseModel(channel_before=test_two_qubit_channel)
        station_b = Station(world=self.world, position=0, BSM_noise_model=two_qubit_noise_model)
        pair_1b = Pair(world=self.world, qubits=[left_station.create_qubit(), station_b.create_qubit()], initial_state=test_state)
        pair_2b = Pair(world=self.world, qubits=[station_b.create_qubit(), right_station.create_qubit()], initial_state=test_state)
        ...
        station_c = Station(world=self.world, position=0, BSM_noise_model=replace_noise_model)
        pair_1c = Pair(world=self.world, qubits=[left_station.create_qubit(), station_c.create_qubit()], initial_state=test_state)
        pair_2c = Pair(world=self.world, qubits=[station_c.create_qubit(), right_station.create_qubit()], initial_state=test_state)
        ...
        after_noise_model = NoiseModel(channel_after=test_two_qubit_channel)
        station_d = Station(world=self.world, position=0, BSM_noise_model=after_noise_model)
        pair_1d = Pair(world=self.world, qubits=[left_station.create_qubit(), station_d.create_qubit()], initial_state=test_state)
        pair_2d = Pair(world=self.world, qubits=[station_d.create_qubit(), right_station.create_qubit()], initial_state=test_state)
