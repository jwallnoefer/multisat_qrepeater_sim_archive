...
class TestDecoherence(unittest.TestCase):
    ...
    def test_single_station_noisy(self):
        dephasing_time = 50
        error_channel = _time_based_channel(epsilon_channel=x_noise_channel, dephasing_time=dephasing_time)
        stations = [Station(world=self.world, id=0, position=0, memory_noise=None), Station(world=self.world, id=1, position=200, memory_noise=error_channel)]
        qubits = [station.create_qubit() for station in stations]
        random_state = np.random.rand(4, 4)
        random_state = (random_state + mat.H(random_state))/ 2 / np.trace(random_state) # make symmetric and normalize
        ...
        trusted_state = _apply_x_noise_on_second_qubit(rho=trusted_state, epsilon=my_epsilon)
    ...
    def test_both_stations_noisy(self):
        dephasing_time = 50
        error_channel1 = _time_based_channel(epsilon_channel=z_noise_channel, dephasing_time=dephasing_time)
        error_channel2 = _time_based_channel(epsilon_channel=x_noise_channel, dephasing_time=dephasing_time)
        stations = [Station(world=self.world, id=0, position=0, memory_noise=error_channel1), Station(world=self.world, id=1, position=200, memory_noise=error_channel2)]
        qubits = [station.create_qubit() for station in stations]
        random_state = np.random.rand(4, 4)
        random_state = (random_state + mat.H(random_state))/ 2 / np.trace(random_state) # make symmetric and normalize
        pair = Pair(world=self.world, qubits=qubits, initial_state=random_state)
        ...
        pair.update_time()
        my_epsilon = (1 - np.exp(-time_interval/dephasing_time)) / 2
        trusted_state = _apply_z_noise_on_first_qubit(rho=trusted_state, epsilon=my_epsilon)
        trusted_state = _apply_x_noise_on_second_qubit(rho=trusted_state, epsilon=my_epsilon)
        self.assertTrue(np.allclose(pair.state, trusted_state))
        # do so many times
        def test_again(trusted_state):
