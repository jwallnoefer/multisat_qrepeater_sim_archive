...
import numpy as np
from libs.aux_functions import apply_single_qubit_map, w_noise_channel, apply_m_qubit_map
...
def _two_qubit_local_wnoise(rho, p):
    rho = apply_single_qubit_map(map_func=_single_qubit_wnoise, qubit_index=0, rho=rho, p=p)
    rho = apply_single_qubit_map(map_func=_single_qubit_wnoise, qubit_index=1, rho=rho, p=p)
    return rho
...
class TestAuxFunctions(unittest.TestCase):
    ...
    def test_apply_m_qubit_map(self):
        # test that parameters are passed through correctly
        two_qubit_array = np.zeros((4, 4), dtype=np.float)
        test_func = MagicMock(return_value=two_qubit_array)
        rho = np.zeros((8, 8), dtype=np.float)
        apply_m_qubit_map(test_func, [0, 1], rho, 3, 7, k=30, my_other_keyword="da_string")
        self.assertEqual(test_func.call_args[0][1:], (3, 7))
        self.assertEqual(test_func.call_args[1], {"k": 30, "my_other_keyword": "da_string"})
        ...
        p = 0.9
        qubit_indices = [1, 3]
        trusted_way = mat.wnoise(rho=mat.wnoise(rho=test_state, n=qubit_indices[0], p=p), n=qubit_indices[1], p=p)
        new_way = apply_m_qubit_map(map_func=_two_qubit_local_wnoise, qubit_indices=qubit_indices, rho=test_state, p=p)
        self.assertTrue(np.allclose(trusted_way, new_way))
