    ...
    def print_status(self, filter=None, max_display=12):
        ...
        if filter is None:
            ...
            if num_events > max_display:
                print(f"There are {num_events} in the event queue. Use world.print_status(\"Event\") to display all. Here are the first {max_display}:")
            ...
            for k, v in self.world_objects.items():
                ...
                if len(v) > max_display:
                    print(f"There are {len(v)} {k}-Objects. Use world.print_status(\"{k}\") to display all. Here are the first {max_display}:")
